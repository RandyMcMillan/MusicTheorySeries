...
@synthesize radius, fillColor, strokeColor, lineWidth, tapColor,
tapStrokeColor, noGradient;
...
- (void)drawRect:(CGRect)rect
{
    ...
    // You could also do this with a custom getter. If the value is nil,
    ...
    // The corner radius can't be more than half of either dimension of the
    // rect, or else it will overlap.
    ...
    if (!self.tapColor) {
        self.tapColor =
            [UIColor colorWithRed:0.0 green:0.36470588 blue:0.96078431 alpha:
            1.0
            ];
    }

    float x = lineWidth / 2 + .5;   // The + .5 is to adjust for clipping
                                    // of antialiasing. This is kind of a
    ...
    // 3. Stroke
    //
    // You can't just fill and stroke it simultaneously, because the fill
    // has a gradient when tapped.
    ...
    CGPathAddArc(myPath, NULL, width - radius, y + radius, radius,
        radians(
            -90), 0, 0);
    CGPathAddLineToPoint(myPath, NULL, width, height - radius);
    CGPathAddArc(myPath, NULL, width - radius, height - radius, radius,
        radians(
            0), radians(90), 0);
    CGPathAddLineToPoint(myPath, NULL, x + radius, height);
    CGPathAddArc(myPath, NULL, x + radius, height - radius, radius,
        radians(
            90), radians(180), 0);
    CGPathAddLineToPoint(myPath, NULL, x, y + radius);
    CGPathAddArc(myPath, NULL, x + radius, y + radius, radius,
        radians(180), radians(270), 0);
    ...
    if (!noGradient) {
        ...
        myGradient =
            CGGradientCreateWithColorComponents(myColorspace, components,
            locations,
            num_locations);
