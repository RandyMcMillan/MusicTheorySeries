...
@property (nonatomic, retain) UIImageView         *cellSeperatorImageView;
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
        ...
        if (self) {
                self.cellTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(TITLE_LABEL_ORIGIN_X, TITLE_LABEL_ORIGIN_Y, TITLE_LABEL_SIZE_WIDTH, TITLE_LABEL_SIZE_HEIGHT)];
                cellTitleLabel.textColor = [UIColor colorWithRed:0.725 green:0.757 blue:0.816 alpha:1.000]; //[UIColor blackColor];
                cellTitleLabel.font = [UIFont fontWithName:@"Helvetica" size:18.0];
                ...
                self.cellImageView = [[UIImageView alloc] initWithFrame:CGRectMake(IMAGE_ICON_ORIGIN_X, IMAGE_ICON_ORIGIN_Y, IMAGE_ICON_SIZE_WIDTH, IMAGE_ICON_SIZE_HEIGHT)];
                [self.contentView addSubview:cellImageView];
                [cellImageView release];

                self.cellBadgeImageView = [[UIImageView alloc] initWithFrame:CGRectMake(IMAGE_BADGE_ORIGIN_X, 0, IMAGE_BADGE_SIZE_WIDTH, IMAGE_BADGE_SIZE_HEIGHT)];

                //NSLog(@"Configure the badge icon here or remove everything associated with badge");
//        cellBadgeImageView.image = [UIImage imageNamed:@"NotificationICON"];
                cellBadgeImageView.hidden = YES;
                [self.contentView addSubview:cellBadgeImageView];
                [cellBadgeImageView release];

                self.cellBadgeTitle = [[UILabel alloc] initWithFrame:CGRectMake(IMAGE_BADGE_ORIGIN_X - 1, 6, IMAGE_BADGE_SIZE_WIDTH, 12.0)];
                ...
                cellBadgeTitle.font = [UIFont fontWithName:@"Helvetica-Bold" size:12.0];
                [self.contentView addSubview:cellBadgeTitle];
                [cellBadgeTitle release];

//        UIImageView *selectedBGImageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"menu_selected_cell"]];
//        selectedBGImageView.frame = self.frame;
//        self.selectedBackgroundView = selectedBGImageView;
//        [selectedBGImageView autorelease];

//                self.selectedColoredView = [[UIView alloc] initWithFrame:CGRectMake(280 - SELECTED_CELL_INDICATOR_WIDTH, 0, SELECTED_CELL_INDICATOR_WIDTH, self.frame.size.height)];
...
- (void)setSubject:(MenuViewItem *)aSubject {
        if (aSubject != subject) {
                [aSubject retain];
                [subject release];
                subject = aSubject;
...
- (void)setSelected:(BOOL)selected animated:(BOOL)animated {
        ...
        if (selected) {
            self.contentView.backgroundColor = [UIColor colorWithRed:0.122 green:0.141 blue:0.176 alpha:1.000];//[UIColor colorWithRed:0.078 green:0.086 blue:0.118 alpha:1.000];// CUSTOMIZE SELECTED STATE HERE
            ...
            self.contentView.backgroundColor =  [UIColor colorWithRed:0.224 green:0.259 blue:0.329 alpha:1.000];//[UIColor colorWithRed:0.145 green:0.165 blue:0.220 alpha:1.000];//[UIColor redColor]; // CUSTOMIZE UNSELECTED STATE HERE
                selectedColoredView.hidden = YES;
...
- (void)triggerBalloonAnimation {
        cellBadgeImageView.transform = CGAffineTransformScale(CGAffineTransformIdentity, 0.001, 0.001);
        [UIView beginAnimations:nil context:nil];
        [UIView setAnimationDuration:0.3/1.5];
        [UIView setAnimationDelegate:self];
        [UIView setAnimationDidStopSelector:@selector(bounce1AnimationStopped)];
        cellBadgeImageView.transform = CGAffineTransformScale(CGAffineTransformIdentity, 1.1, 1.1);
...
- (void)bounce1AnimationStopped {
        ...
        cellBadgeImageView.transform = CGAffineTransformScale(CGAffineTransformIdentity, 0.9, 0.9);
        [UIView commitAnimations];
}
