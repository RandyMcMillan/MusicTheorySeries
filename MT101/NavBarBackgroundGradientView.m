...
{
    CGRect imageBounds = CGRectMake(0.0, 0.0, self.bounds.size.width,
    ...
    drawRect    = CGRectMake(0.0, 0.0, self.bounds.size.width,
        self.bounds.size.height);
    drawRect.origin.x =
        (round(resolution * drawRect.origin.x +
            alignStroke) - alignStroke) / resolution;
    drawRect.origin.y =
        (round(resolution * drawRect.origin.y +
            alignStroke) - alignStroke) / resolution;
    drawRect.size.width =
        round(resolution * drawRect.size.width) / resolution;
    drawRect.size.height =
        round(resolution * drawRect.size.height) / resolution;
    ...
    [colors addObject:(id)[color CGColor]];
    locations[4]    = 0.465;
    gradient        = CGGradientCreateWithColors(space, (CFArrayRef)colors,
        locations);
