...
// Data structure that defines a single audio file, related to a given
// tone
typedef struct {
  AudioUnitSampleType *audioData;           // the complete left (or
                                            // mono) channel of audio
                                            // data read from an audio
                                            // file
...
//    which gets invoked by a Mixer unit input bus when it needs more
// audio to play.
typedef struct {
  AudioUnitSampleType     *audioData;               // the complete left
                                                    // (or mono) channel
                                                    // of audio data
                                                    // read from an
                                                    // audio file
  UInt32                  frameCount;               // the total number
                                                    // of frames in the
                                                    // audio data
  UInt32                  sampleNumber;             // the next audio
                                                    // sample to play
  BOOL                    hasAudio;                 //
...
@interface MixerHostAudio : NSObject <AVAudioSessionDelegate> {
  ...
  // Before using an AudioStreamBasicDescription struct you must
  // initialize it to 0. However, because these ASBDs
  // are declared in external storage, they are automatically
  // initialized to 0.
...
//@property (readwrite)           AudioStreamBasicDescription
// stereoStreamFormat;
@property (readwrite)           AudioStreamBasicDescription
  monoStreamFormat;
@property (readwrite)           Float64
  graphSampleRate;
